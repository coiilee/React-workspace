...
DB나 전달받고 전달할 값을 어떻게 받고 전달할 것인지에 대한 상태를 부모에 작성
...
//props나 abc,xyz와 같은 매개변수 명칭을 하나 택해서 작성하고
//예를 들어 props 라는 매개변수를 사용한다면
//props.name , props,inheritance 와 같이 호출해서 사용할 수 있음
//여기서 props와 같이 단일값으로 가져올 때는 {}를 사용하지 않음. 다수를 가져올 때 중괄호 사용함
// (props) 작성 후 , props.name props.inheritance
// const Child2 = ({name  , inheritance}) => {
const Child2 = (props) => { //DB에서 엄청나게 많은 컬럼을 들고 왔다는 표기
    return(
        <div>
...
const Parent = () => {
    return(
        <div>
            <h1>👨‍👩‍👧‍👦 부모의 재산 상속</h1>
            {/*     데이터베이스에서 전달받은  */}
            <Child1 name="이름" inheritance={"재산"} />
            <Child1 name="홍길동" inheritance={10} />
            <Child2 name="김철수" inheritance={5} />
            <Child2 name="이영희" inheritance={20} />
